...
(defn card-template [state-atom {:keys [path options func] :as card}]
  (d/div {:class "com-rigsomelight-devcard" :key (path->unique-card-id path)}
...
(defn breadcrumbs-templ [crumbs state-atom]
  (let [counter (atom 0)
        sep-fn  (fn [_] (d/span {:class "com-rigsomelight-devcards-breadcrumb-sep" :key (do (swap! counter inc) @counter)}
                          "/"))]

    (d/div
      {:class "com-rigsomelight-devcards-card-base com-rigsomelight-devcards-breadcrumbs com-rigsomelight-devcards-typog"
        ...
        (interleave
          ...
          (map (fn [[n path]]
                 (d/span
                   ...
                   (d/a
                     {:href    "#"
                      :class "com-rigsomelight-devcards_set-current-path"
...
(defn dir-links [dirs state-atom]
  (when-not (empty? dirs)
    (d/div {:class "com-rigsomelight-devcards-list-group com-rigsomelight-devcards-typog"}
      (map (fn [[key child-tree]]
             (d/a
               {:href  "#"
                :class "com-rigsomelight-devcards-list-group-item"
                :key   (str key)
                :onClick (prevent-> (fn [e] (navigate-to-path key state-atom))) }
               (d/span
                 {:style {:float "right"}
                  :class "com-rigsomelight-devcards-badge"}
                 (count child-tree))
               (d/span " " (name key))))
...
(defn main-template [state-atom]
  (let [data @state-atom]
    (d/div
      ...
      (d/div {:class "com-rigsomelight-devcards-container"}
        ...
        (d/div (main-cards-template state-atom))))))
